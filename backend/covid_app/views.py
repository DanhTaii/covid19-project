...
class WorldMapAPIView(APIView):
    def get(self, request):
        ...
        try:
            ...
            logger.error(f"Lỗi đọc file Parquet: {e}")
            return Response({"error": "Lỗi đọc file"}, status=500)

        # Chuẩn hóa cột ngày
        df["date"] = pd.to_datetime(df["date"], errors="coerce")

        # Loại bỏ các dòng không phải quốc gia
        exclude = [
            "World", "Europe", "European Union", "High income", "Low income",
            "Upper middle income", "Lower middle income", "Asia", "Africa",
            "North America", "South America", "Oceania", "International"
        ]
        df = df[~df["location"].isin(exclude)]

        # Chọn cột dữ liệu theo chế độ
        ...
        print("TOP 1:", countries[0] if countries else "RỖNG")
